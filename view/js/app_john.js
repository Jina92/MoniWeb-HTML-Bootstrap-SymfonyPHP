...
function fetchlogin(evt) {
    ...
    ffd.append('username', loginuser.value);
    ffd.append('password', loginpass.value);
    fetch('http://localhost/proj-template-docker/api/api.php?action=login',
    {
        method: 'POST',
        body: ffd,
        credentials: 'include'
...
function fetchregister(evt) {
    ...
    fetch('http://localhost/proj-template-docker/api/api.php?action=register', 
    {
        method: 'POST',
        body: fd,
        credentials: 'include'
...
function fetchaccountexists(evt) {
    if(evt.srcElement.value.length > 3) {
        fetch('http://localhost/proj-template-docker/api/api.php?action=accountexists&username='+ evt.srcElement.value, 
        {
            method: 'GET',
            credentials: 'include'
        })
...
function fetchisloggedin(evt) {
    fetch('http://localhost/proj-template-docker/api/api.php?action=isloggedin', 
    {
        method: 'GET',
        credentials: 'include'
    })
...
function fetchlogout(evt) {
    fetch('http://localhost/proj-template-docker/api/api.php?action=logout', 
    {
        method: 'GET',
        credentials: 'include'
    })
